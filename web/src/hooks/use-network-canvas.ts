...
export function useNetworkCanvas(
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (vignette) {
        const v = ctx.createRadialGradient(W / 2, H / 2, H * 0.15, W / 2, H / 2, H * 0.85);
      ...
      for (const n of nodes) {
        if (n.glow) {
          ...
          g.addColorStop(0, "rgba(110,231,179,0.35)"); g.addColorStop(1, "rgba(110,231,179,0)");
          ctx.beginPath(); ctx.arc(n.x, n.y, n.r * 7, 0, Math.PI * 2);
          ctx.fillStyle = g; ctx.fill();
        }
        ctx.beginPath(); ctx.arc(n.x, n.y, n.r, 0, Math.PI * 2);
        ctx.fillStyle = n.col; ctx.shadowColor = n.col;
        ctx.shadowBlur = n.glow ? 10 : 4; ctx.fill(); ctx.shadowBlur = 0;
      }
